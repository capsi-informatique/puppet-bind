...
define bind::zone (
  ...
  case $ensure {
    'present': {
      ...
      case $zone_type {
        'master': {
          if $zone_contact == undef or $zone_ns.length == 0 or $zone_serial == undef or $zone_ttl == undef {
            fail('$zone_contact, $zone_ns, $zone_serial and $zone_ttl are required on master zone.')
          }
